...
class SingleTypeKVCacheManager(ABC):
    ...
    def remove_skipped_blocks(self, request_id: str,
        ...
        raise NotImplementedError
    # ---------- 引用计数 ----------
    def increase_refcounts(self, phys_ids: list[int]) -> None:
        for bid in phys_ids:
            self._blocks[bid].refcount += 1

    def decrease_refcounts(self, phys_ids: list[int]) -> None:
        for bid in phys_ids:
            blk = self._blocks[bid]
            blk.refcount -= 1
            if blk.refcount == 0:
                # 用你现有的释放路径；free_list 或 allocator 的 free API
                self._free_list.append(bid)

    # ---------- 写时分配（当共享块将被写入时调用） ----------
    def ensure_writable_block(self, table, logical_idx: int) -> None:
        bid = table._block_ids[logical_idx]
        blk = self._blocks[bid]
        if blk.refcount > 1:
            # 分配形状一致的新块（替换方法名为你项目现有的分配/复制 API）
            new_bid = self._allocate_block_like(bid)
            self._copy_block_data(dst=new_bid, src=bid)  # 如不需要也可省略
            # 当前视图切换到新块；调整引用计数
            table._block_ids[logical_idx] = new_bid
            blk.refcount -= 1
            self._blocks[new_bid].refcount = 1
