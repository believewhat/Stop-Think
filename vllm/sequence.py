...
class Sequence:
    ...
    def spawn_child_from_block_table(self, new_seq_id: str, block_table) -> "Sequence":
        child = self.__class__.__new__(self.__class__)
        # 复制必要的轻量属性
        child.seq_id = new_seq_id
        child.tokens = list(self.tokens)         # 共享文本语义，token 列表可独立
        child.block_table = block_table          # 共享/视图化的 KV
        child.finished = False
        # 复制 LoRA/prompt adapter/rope scaling 等必要上下文（若有）
        child.lora_info = getattr(self, "lora_info", None)
        child.prompt_adapter_info = getattr(self, "prompt_adapter_info", None)
        # ...其它 engine 运行所需的元数据，按你们现有 Sequence 字段补全
        return child
