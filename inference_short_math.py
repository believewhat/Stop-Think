...
async def amain(args):
    ...
    if args.cls_model_path:
        ...
        print("[CLS] No --cls_model_path provided; classifier will NOT be loaded.",
              flush=True)
    ...
    try:
        ...
        out["gt_text"] = out.apply(
            lambda r: (
                ...
                else (r["answer"] if isinstance(r["answer"], str) else "")
            ),
            axis=1,
        )
        ...
        # 简单 sanity check 表
        dbg = out.loc[mask_eval, ["unique_id", "pred_boxed", "gt_text"]].head(12).copy()
        if len(dbg) > 0:
