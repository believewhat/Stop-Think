...
def collect_xy_from_non_tensor(non_tensor_batch, key_x='feature_x', key_y='feature_y'):
    """把 batch.non_tensor_batch[key_x/key_y] 里面每卡的一包 X/y 拼起来"""
    ...
    for i in range(len(fx)):
        ...
        if n == 0:
            continue
        X_parts.append(Xi[:n].astype(np.float32, copy=False))
        y_parts.append(yi[:n].astype(np.int64, copy=False))


    if not X_parts:
        return np.zeros((0, 30), np.float32), np.zeros((0,), np.int64)  # 你的特征是 30 维
...
def dedup_by_X(X, y, decimals=8, strategy='majority'):
    ...
    X_uniq = X[idx]
    y_uniq = np.empty(len(idx), dtype=y.dtype)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if 'multi_modal_inputs' in batch.non_tensor_batch.keys():
                    ...
                    gen_batch = batch.pop(
                        batch_keys=['input_ids', 'attention_mask', 'position_ids'],
                        non_tensor_batch_keys=['raw_prompt_ids', 'correct_think', 'solution', 'reward_model'],
                    )
                ...
                with _timer('step', timing_raw):
                    ...
                    X_all, y_all = collect_xy_from_non_tensor(batch.non_tensor_batch, 'feature_x', 'feature_y')
                    X_uniq, y_uniq, info = dedup_by_X(X_all, y_all, decimals=8, strategy='majority')
                    if len(set(y_uniq)) > 1:
                        if self.train_flag:
                            self.clf = lgb.train(self.params_clf, lgb.Dataset(X_uniq, y_uniq), num_boost_round=50, init_model=self.clf)
                        else:
                            self.clf = lgb.train(self.params_clf, lgb.Dataset(X_uniq, y_uniq), num_boost_round=50)
                            self.train_flag = 1
                    ...
                    if self.config.trainer.save_freq > 0 and ( is_last_step or \
                        ...
                        local_global_step_folder = os.path.join(self.config.trainer.default_local_dir,
                                                f'global_step_{self.global_steps}')
                        self.clf.save_model(os.path.join(local_global_step_folder, 'lgb_booster.txt'))
