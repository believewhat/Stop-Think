...
class MyRLHFDataset(RLHFDataset):
    def __getitem__(self, item):
        ...
        row_dict["solution"] = self.dataframe[item]['solution']
        row_dict["options"] = self.dataframe[item]['options']
        """
        if "correct_think" in row_dict and isinstance(row_dict["correct_think"], str):
            correct_ids = self.tokenizer.encode(row_dict.pop("correct_think"), add_special_tokens=False)
            row_dict["correct_think_input_ids"] = correct_ids[0]
        """
        return row_dict


def my_collate_fn(features):
    from verl.utils.dataset.rl_dataset import collate_fn as default_collate
