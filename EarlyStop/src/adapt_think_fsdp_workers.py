...
class AdaptThinkActorRolloutRefWorker(Worker):
    """
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == 'vllm':
            ...
            raise NotImplementedError("rollout_name must be 'vllm'")
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelPPOActor(
                                ...
                                #ref_model=self.ref_policy.actor_module if self._is_ref else None,   # <--- 加上这行！
                                #dpo_beta=getattr(self.config.actor, "dpo_beta", 0.1),   # 可选，DPO权重超参
                                #dpo_weight=getattr(self.config.actor, "dpo_weight", 5.0),  # 可选，DPO权重超参
                                #tokenizer=self.tokenizer
                            )
    ...
    def update_actor(self, data: DataProto):
        ...
        opt = self.actor_optimizer
        param_count = sum(p.numel() for group in opt.param_groups for p in group['params'])
        print(f"Optimizer will init {param_count / 1e6:.2f}M parameters.")

        estimated_mem = param_count * 12 / (1024 ** 3)
        print(f"[DEBUG] Estimated Optimizer memory: {estimated_mem:.2f} GB")
